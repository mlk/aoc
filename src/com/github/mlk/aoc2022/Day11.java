...
public class Day11 {
    ...
    record Monkey(int number, List<Integer> items, Function<Integer, Integer> operation, int divisibleTest, int trueMonkey, int falseMonkey) {
        static Monkey parse(String monkey) throws ScriptException {
            ...
            List<Integer> items = Arrays.stream(lines[1].substring(18).split(",")).map(String::trim).map(Integer::parseInt).toList();
            ...
            Function<Integer, Integer> operation = (oldValue) -> {
                ...
                try {
                    return ((Number) engine.eval("Math.floor( (Math.floor(" + calc + ")) / 3)")).intValue();
                } catch (Exception e) {
        ...
        public static void main(String... arg) throws ScriptException {
            ...
            for(int i = 0; i< 20; i++) {
                for(Monkey monkey : monkeys) {
                    for(int item : monkey.items()) {
                        ...
                        int worry = monkey.operation().apply(item);
            ...
            System.out.println(sorted[sorted.length-1] * sorted[sorted.length-2]);
        }
